...
const EMBEDDED_SIGSTORE_TRUST_ROOT: &[u8] = include_bytes!(concat!(
    ...
    "/trust/sigstore-trusted-root.json"
));
const EMBEDDED_SIGSTORE_TRUST_ROOT_PROVENANCE: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/trust/sigstore-trusted-root.provenance.json"
));
...
mod tests {
    ...
    async fn v03_real_cosign_plugin_bundle_verifies_with_rfc3161_timestamp() {
        ...
        let bundle = include_bytes!("../test-fixtures/fanzub-catalog-v2.1.0.sigstore.json");
...
mod binding_tests {
    ...
    fn embedded_fulcio_pems() -> Vec<String> {
        let material = load_embedded_sigstore_trust_material().expect("load offline trust fixture");
        material
            .material
            .fulcio_chains
            .iter()
            .flat_map(|chain| std::iter::once(&chain.anchor_der).chain(chain.intermediates.iter()))
            .map(|cert| pem_encode_certificate(cert.as_ref()))
            .collect()
    }
    ...
    #[test]
    fn rekor_hashedrekord_binding_accepts_matching_artifact_signature_and_certificate() {
        let cert_pems = embedded_fulcio_pems();
        let cert_pem = cert_pems
            .first()
    ...
    #[test]
    fn rekor_hashedrekord_binding_rejects_altered_artifact_certificate_and_body() {
        let cert_pems = embedded_fulcio_pems();
